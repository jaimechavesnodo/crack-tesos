...
- **Server-side tool `web_search` en una sola llamada HTTP (sin loop manual)**: a diferencia de los user-defined tools, `web_search_20260209` (y otros tools server-side: code_execution, web_fetch) corren en la infraestructura de Anthropic — Claude busca, lee resultados y escribe la respuesta final, todo dentro de UNA sola request/response. No requiere header beta ni segunda llamada para enviar tool_result. Se declara así en el body: `tools: [{type: 'web_search_20260209', name: 'web_search'}]`. Usado en WF-14 para que el análisis del email investigue el estado real del Mundial 2026 (el modelo Haiku no tiene ese conocimiento en su entrenamiento).
- **Con tool use, la respuesta de Claude ya NO es `content[0].text`**: cuando hay tool calls intercalados, `content[0]` puede ser un bloque `server_tool_use` o `web_search_tool_result`, no texto. Hay que filtrar por `type==='text'` y tomar el ÚLTIMO bloque de texto (la respuesta final viene después de las búsquedas): `(claudeResp.content||[]).filter(b=>b.type==='text').pop()?.text`. Ver "Construir Email HTML" en WF-14.
...
## Email WF-14 — banner "Noticia Mundial"

El prompt de Claude en "Preparar Contexto" pide una 6ta clave JSON `mundial: {titular, texto}` que compara el avance de Crack Tesos con el estado real del Mundial 2026, investigado vía `web_search`. Se renderiza como banner oscuro estilo noticia justo después del header, antes del pre-header. Si Claude no devuelve búsquedas útiles o el JSON falla, cae a los defaults vacíos sin romper el resto del email (mismo patrón try/catch que los otros 5 análisis).
